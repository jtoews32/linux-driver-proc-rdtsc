...
unsigned long long  stamp1, stamp2, cycles;
...
{
	int	 len = 0;
	int start = jiffies_64;
	int timeout = start + HZ/10 ;

	// asm(" cli \n rdtsc ");
	asm("rdtsc");
	asm(" mov %%eax, stamp1 \n mov %%edx, stamp1+4 " : : : "ax", "dx" );

	while(jiffies_64 < timeout) {} // Go for 1/10 of a second
